...
/*
 * Lets the Control page scroll the scripture text on this display
 * remotely (via a "scrollCommand" localStorage key), so the operator
 * never has to touch this window directly to read a long passage.
 */
function handleRemoteScroll(amount) {
    const scriptureContainer =
        document.getElementById("scriptureContainer");

    if (!scriptureContainer) return;

    scriptureContainer.scrollBy({
        top: amount,
        behavior: "smooth"
    });
}
...
window.addEventListener("storage", async (event) => {
    ...
    if (event.key === "scrollCommand" && event.newValue) {
        try {
            const command = JSON.parse(event.newValue);
            handleRemoteScroll(command.amount);
        } catch (error) {
            console.error("Unable to read scroll command.", error);
        }
    }
