 ...
 function returnToWelcome() {
    ...
    (function enablePreviewResizing() {
        ...
        function moveResize(clientX) {
            ...
            if (anchor === "right") {
                // Left edge pinned — dragging right grows the box.
                newWidth = startWidth + deltaX;
                newLeft = startLeft;
            } else {
                // Right edge pinned — dragging left grows the box.
                newWidth = startWidth - deltaX;
            ...
            if (anchor === "left") {
                const widthDelta = newWidth - clampedWidth;
                newLeft = newLeft + widthDelta;
            }
    ...
    function broadcastScrollPosition(position) {
        localStorage.setItem("scrollPosition", String(position));
    }
    ...
    function scrollPreviewBy(amount) {
        const container = getPreviewScriptureContainer();
        if (!container) return;

        container.scrollBy({ top: amount, behavior: "smooth" });

        // scrollBy is async-ish (smooth animates), so read back the
        // final position shortly after to broadcast once settled.
        setTimeout(() => {
