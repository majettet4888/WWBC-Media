...
focusReferenceInput();
 ...
 function returnToWelcome() {
    ...
    // Resizing — drag the corner handle to make the preview bigger
    // or smaller. The iframe is re-scaled (not just cropped) so
    ...
    (function enablePreviewResizing() {
        ...
        function applyWidth(width) {
            ...
            livePreviewBox.style.width = `${clamped}px`;
            livePreviewBox.style.setProperty(
                "--preview-scale",
                clamped / NATIVE_WIDTH
            );
        }
        ...
        function startResize(clientX) {
            resizing = true;
            startX = clientX;
            startWidth = livePreviewBox.offsetWidth;
        }
        ...
        previewResizeHandle.addEventListener("touchstart", (e) => {
        ...
        applyWidth(livePreviewBox.offsetWidth || 340);
    })();

    /*
     * Remote scroll — sends a scroll command to the TV(s) so the
     * operator never has to move the cursor onto that window (or
     * turn around) just to read the rest of a long passage.
     */
    function sendScrollCommand(amount) {
        localStorage.setItem(
            "scrollCommand",
            JSON.stringify({ amount, ts: Date.now() })
        );
    }

    scrollUpButton.addEventListener("click", () => {
        sendScrollCommand(-250);
    });
